...
def run_heuristics(snap, db_conn=None) -> list[dict]:
    ...
    if snap.cpu_total_pct > 85 and user_procs:
        ...
        if new_nice != t.nice:
            ...
            ev = {"rule":"high_cpu","pid":t.pid,"name":t.name,"action":"renice",
                  "old_nice":t.nice,"new_nice":new_nice,
                  "reason":f"CPU {snap.cpu_total_pct:.1f}% > 85%","applied":ok}
            events.append(ev)
            if db_conn:
                log_scheduler_event(db_conn, t.pid, "renice", t.nice, new_nice, ev["reason"])
    ...
    if snap.mem_total_kb > 0:
        ...
        if mem_pct > 90:
            ...
            if top:
                events.append({"rule":"high_mem","pid":top[0].pid,"name":top[0].name,
                                "action":"kill_suggestion","reason":f"RAM {mem_pct:.1f}% > 90%"})
    ...
    if snap.load_avg_1 > num_cores * 1.5 and user_procs:
        ...
        if new_nice != t.nice:
            ok = renice(t.pid, new_nice)
            events.append({"rule":"high_load","pid":t.pid,"name":t.name,"action":"renice",
                           "old_nice":t.nice,"new_nice":new_nice,
    ...
    if snap.cpu_total_pct < 20:
        for p in user_procs:
            if p.nice > 0:
                renice(p.pid, 0)
                events.append({"rule":"low_cpu_restore","pid":p.pid,"name":p.name,
